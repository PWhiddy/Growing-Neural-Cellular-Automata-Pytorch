...
class ParticleSystemLongRange:
        ...
        for i in range(start_count):
            self.parts.append(
                Particle(
                    ...
                    np.array([0.04*rnd.random()-0.02, 0.04*rnd.random()-0.02])
                )
            )
    ...
    def pair_force(self, a, b):
        ...
        # 1/r^2 force
        frc = dv*self.repel_strength/(dist*dist*dist+0.1)
        a.vel += frc
        b.vel -= frc
    ...
    def draw(self):
        ...
        for e, part in zip(self.ells, self.parts):
            col = np.clip(
                ParticleSystemLongRange.hsv2rgb([
        ...
        #plt.savefig(f'p_test_lr/t{self.draw_step:06d}.png')
        self.draw_step += 1
