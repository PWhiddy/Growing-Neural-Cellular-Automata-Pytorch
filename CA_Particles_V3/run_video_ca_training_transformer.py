...
from ca_particles import VideoSimulation, CAModelTransformer, CASimulation, CATrainer

if __name__ == '__main__':
    cell_dim = 16
    hidden_dim = 160
    batch_size = 2 #32#24
    train_steps = 4096*9 #9
    learning_rate = 2e-3
    lr_decay_rate = 1024*24
    model_steps_per_video_frame = 2 #8
    step_blocks = 16
    seed = 55
    ...
    if pretrain_path == None:
        ca_model = CAModelTransformer(cell_dim).to(device)
    else:
        ca_model = torch.load(pretrain_path)

    ca_model = torch.compile(ca_model)
    ...
    trainer = CATrainer(
        ...
        seed=seed, gt_reset_interval=10000000, checkpoint_path=f'checkpoints_c{cell_dim}_h{hidden_dim}', checkpoint_interval=9999999
    )
